...
public class PagamentosDAOimpl extends BDconexaoDAO implements PagamentosDAO { // DECLARAÇÃO DA CLASSE "PagamentosDAOimpl" QUE EXTENDE A CLASSE "BDconexaoDAO" E IMPLEMENTA A INTERFACE "PagamentosDAO"
    ...
    public DefaultTableModel mostrar(String buscar) { // DECLARAÇÃO DA CLASSE RESPONSAVEL PELA CONSULTA E RETORNO DOS DADOS DO BANCO     
        ...
        String[] titulos = {"ID Pagamento", "ID Reservas", "Tipo Comprovante", "Num Comprovante", "Total Pagamento", "Data Emissão", "Data Pagamento"}; // DEFINE O TÍTULO DAS COLUNAS NA LISTA QUE APARECE PARA VERIFICAÇÃO EM CADA TELA DO SISTEMA
        String[] registro = new String[7]; // DECLARA A QUANTIDADE DE REGISTROS
        ...
        try { // BLOCO TRY RESPONSÁVEL POR EXECUTAR A CONSULTA DO sSQL
            ...
            while (rs.next()) { // BUSCA CADA UM DOS REGISTROS NO BANCO E ADICIONA EM UMA NOVA LINHA NA TABELA
                ...
                registro[4] = rs.getString("total_pagamento");
                registro[5] = rs.getString("data_emissao");
                registro[6] = rs.getString("data_pagamento");
    ...
    public boolean inserir(Pagamentos dts) { // "dts" É UM OBJETO CRIADO PARA IDENTIFICAR QUE O CÓDIGO ESTÁ DIRETAMENTE ITERADO  AO BANCO
        ...
        sSQL = "insert into tb_pagamentos (id_reserva, tipo_comprovante, num_comprovante, total_pagamento, data_emissao, data_pagamento)"
                + "values(?,?,?,?,?,?)";
        ...
        try { // BLOCO RESPONSÁVEL PELA EXECUÇÃO DAS QUERYS.
            ...
            pst.setDouble(4, dts.getTotal_pagamento());
            pst.setDate(5, java.sql.Date.valueOf(dts.getData_emissao().toString()));
            pst.setDate(6, java.sql.Date.valueOf(dts.getData_pagamento().toString()));
    ...
    public boolean editar(Pagamentos dts) { // "dts" É UM OBJETO CRIADO PARA IDENTIFICAR QUE O CÓDIGO ESTÁ DIRETAMENTE ITERADO  AO BANCO
        ...
        sSQL = "update tb_pagamentos set id_reserva=?, tipo_comprovante=?, num_comprovante=?, total_pagamento=?, data_emissao=?, data_pagamento=?"
                + "where id_pagamento=?";
        ...
        try { // BLOCO RESPONSÁVEL PELA EXECUÇÃO DAS QUERYS.
            ...
            pst.setDate(5, java.sql.Date.valueOf(dts.getData_emissao().toString()));
            pst.setDate(6, java.sql.Date.valueOf(dts.getData_pagamento().toString()));
            pst.setInt(7, dts.getIdpagamento());
